...
namespace DAL
{
    public class TopicDao : CommonDao
    {
        ...
        {
            string strSql = "select topic_id, content, if (topic_type='1','理论类','操作类'), b.TCZ_NAME, answers, create_time, topic_type, topic_category,if (getIsChoose(" + strWheres[" exam_id "] + ",topic_id)='true','已选择','')  as 是否选择  from ex_topic a, tcz_type b where a.TOPIC_CATEGORY = b.TCZ_ID ";
